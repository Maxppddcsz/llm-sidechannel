...
# 
# i.e. In a small time window, like in less than 0.4 s intervals, it is more likely that the system is stable.
# So, at that time, we will send three prompts in a row.
# 
# The attacker can use the tokenizer to generate mis-token prompts.
# Step 1: Trigger the system prompts.
# Step 2: Send original prompts without appended token, record the TTFT.
# Step 3: Send predicated prompts, record the TTFT.
# Step 4: Repeat 10 times, record the average value.
# Step 5: Find whether we find the system prompts. (The differences of average time value between HIT and MISS is more than 5 times larger than that between MISS and MISS)
...
def oracle_check(seperator, prefix, log_token, triggering_prompt, bad_tokens1, bad_tokens2, check_time):
    ...
    for _ in range(check_time):
        ...
        bad_ttft_ret_1.append(bad_ttft_1)
...
def test_ok(seperator, prefix, startlen, temper, test_time, oracle_ids, pace, penalty_table, triggering_prompt):
...
for d in tqdm(range(len(tempers)), desc='temper:'):
    ...
    for k in tqdm(range(len(mylist)), desc='requests:'):
        ...
        result = test_ok(seperator, known_sentence, start, tempers[d], test_time, next_tokens_ids, pace, penalty_table, triggering_prompt)
