...
parser.add_argument('-m', '--model_file', type=str, default="YOUR_FINE_TUNED_MODEL")
parser.add_argument('-t', '--test_prompt', type=str, default="YOUR_TEST_DATASET.pkl")
parser.add_argument('-r', '--result', type=str, default="YOUR_RESULT.pkl")
# Set path for 1, 2, 3, 4
...
def forward(prefix, step, history, temper, penalty_table):
...
def test_ok(seperator, prefix, startlen, temper, test_time, oracle_ids, pace, penalty_table, tpr_fpr):
    ...
    for _ in range(test_time):
        count += 1
        # print('============ Guess Time =================')
        log, log_token = forward(prefix, pace, history, temper, penalty_table)
        history += ([tuple(log[:i+1]) for i in range(len(log))])

        # print(f'history: {history}')
        print(f'log: {repr(log)} log_token: {repr(log_token)}')
...
mylist = []
with open(f"{test_prompt}", "rb") as file:
...
tpr_fpr = [[0.88, 0.10], [0.908, 0.082], [0.910, 0.070], [0.920, 0.07]]
...
for d in tqdm(range(len(tempers)), desc='temper:'):
    ...
    for k in tqdm(range(len(mylist)), desc='requests:'):
        ...
        # Known Info by statistical method.3    
        ...
        result = test_ok(seperator, known_sentence, start, tempers[d], test_time, next_tokens_ids, pace, penalty_table, tpr_fpr)
